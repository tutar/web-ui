...
function AudioBookCard({ message }: { message: any }) {
      ...
      <div className="p-6 flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            onClick={() =>
              setChapterIdx(
                Math.min(message.chapters.length - 1, chapterIdx + 1),
              )
            }
            className="text-theme-text-secondary hover:text-theme-text transition-colors"
          >
        ...
        <div className="flex-1 overflow-auto p-6 flex flex-col gap-6">
          {isNewChat && messages.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-theme-text-secondary h-full min-h-[300px]">
              ...
              <p className="max-w-md text-center text-sm">{t("chat.newChatDesc", "Welcome to AgentOS. Send a message to start a new chat, or pick a skill/connector from the right sidebar to start working.")}</p>
            </div>
          ) : (
            messages.map((msg) => {
                ...
                {isUser ? (
                  ...
                  <div className="flex gap-4 max-w-[80%] w-full">
                    ...
                    <div className="flex flex-col gap-2 w-full min-w-0">
                      <AgentMessage message={{ content: textContent, processSteps: msg.processSteps || [] }} />
                    </div>
                  </div>
                )}
              </div>
          ...
          {session?.status === "running" && messages.length > 0 && messages[messages.length - 1].messageType === "user" && (
            ...
            <div className="p-4 bg-theme-surface-hover rounded border border-theme-border space-y-3 mt-8">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="text-center py-2 bg-theme-surface rounded border border-theme-border">
                  <div className="text-lg font-serif italic text-theme-accent">
                    03
                  </div>
          ...
          <div className="flex-1 flex flex-col items-center py-4 gap-4 overflow-y-auto">
            {SUB_AGENTS.map((agent) => (
              <div 
                ...
                style={{
                  borderColor: agent.status === "completed" ? "#10b981" : agent.status === "in-progress" ? "var(--accent-color)" : "var(--border-muted)",
