			...
			>
							...
							{isRecentChatsExpanded && (
								<div className="flex flex-col gap-0.5 mt-1">
									{sessions.map((chat) => (
										...
										>
											<span className="truncate w-full block font-medium">{chat.sessionName}</span>
				...
				<div className="w-full p-2 md:p-3 border-t border-theme-border">
					{!isCollapsed && currentUser ? (
						<div className="px-3 py-2 text-xs text-theme-text-secondary border-b border-theme-border mb-2">
							<div className="font-medium text-theme-text truncate">{currentUser.username}</div>
							<button onClick={() => void logout()} className="mt-1 text-theme-accent">
					...
					>
						...
						{!isCollapsed && <span className="transition-opacity duration-300">{t("layout.settings")}</span>}
					</button>
				</div>
