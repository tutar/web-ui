	...
	const renderTree = (nodes: any[], level = 0) => {
		return nodes.map((node) => (
          ...
          `}
				...
				>
					{node.type === "folder" ? (
						expandedDirs[node.id] ? (
							<ChevronDown strokeWidth={2} className="w-3.5 h-3.5 text-theme-text-muted" />
						) : (
							<ChevronRight strokeWidth={2} className="w-3.5 h-3.5 text-theme-text-muted" />
						)
					) : (
						<div className="w-3.5 h-3.5 flex-shrink-0" /> /* Placeholder to align files */
					)}
					{node.type === "folder" ? (
						<Folder fill="currentColor" strokeWidth={0} className="w-4 h-4 text-theme-text-muted" />
					) : (
						<FileText
							strokeWidth={1.5}
				...
				{node.type === "folder" && expandedDirs[node.id] && node.children && renderTree(node.children, level + 1)}
			</div>
		));
	};

	const selectedFile = selectedNodeId !== skill.id ? findNode(skill.files, selectedNodeId) : null;
	const isRenderableFile = selectedFile && (selectedFile.name.endsWith(".md") || selectedFile.name.endsWith(".html"));
			...
			<div className="w-72 border-r border-theme-border flex flex-col bg-theme-base flex-shrink-0 relative z-10">
				...
				<div className="flex-1 overflow-y-auto pb-4">
                  ...
                  ${selectedNodeId === skill.id ? "bg-theme-surface-active text-theme-text" : "hover:bg-theme-surface-hover text-theme-text-secondary hover:text-theme-text"}`}
							...
							>
								{isSkillOpen ? (
									<ChevronDown strokeWidth={2} className="w-3.5 h-3.5 text-theme-text-secondary" />
								) : (
									<ChevronRight strokeWidth={2} className="w-3.5 h-3.5 text-theme-text-secondary" />
								)}
							...
							{/* Files */}
							{isSkillOpen && (
								<div className="mt-1 flex flex-col font-mono text-[13px]">{renderTree(skill.files)}</div>
							)}
						</div>
					)}
				</div>
					...
					<>
						<div className="px-8 py-6 border-b border-theme-border flex flex-col gap-6">
							<div className="flex justify-between items-center">
								<h2 className="text-xl font-bold text-theme-text">{skill.name}</h2>
							...
							<div className="flex gap-16 text-sm">
								<div>
									...
									<div className="text-theme-text font-medium text-sm">{skill.providedBy}</div>
								</div>
								<div>
									<div className="text-theme-text-secondary text-[11px] uppercase tracking-wider font-semibold mb-1">
										Trigger
									</div>
									<div className="text-theme-text font-medium text-sm">{skill.trigger}</div>
								</div>
							</div>
							...
							<div>
								...
								<p className="text-theme-text text-sm leading-relaxed max-w-4xl">{skill.description}</p>
							</div>
						</div>
							...
							<div className="max-w-4xl mx-auto border border-theme-border bg-theme-surface rounded-xl overflow-hidden min-h-[calc(100vh-12rem)] shadow-sm">
								{isRenderableFile && viewMode === "preview" ? (
									<div className="p-8 text-theme-text text-base leading-relaxed space-y-4">
										<p>
											The skill creator is liable to be used by people across a wide range of familiarity
											with coding jargon. If you haven't heard (and how could you, it's only very recently
											that it started), there's a trend now where the power of Claude is inspiring plumbers
											to open up their terminals, parents and grandparents to google "how to install npm". On
											the other hand, the bulk of users are probably fairly computer-literate.
										</p>
										<p>
										...
										<ul className="list-disc pl-6 space-y-2 mt-4 marker:text-theme-text-secondary">
											<li>"evaluation" and "benchmark" are borderline, but OK</li>
											<li>
										...
										<p className="mt-4">
											It's OK to briefly explain terms if you're in doubt, and feel free to clarify terms
											with a short definition if you're unsure if the user will get it.
										</p>
									</div>
								) : (
									<div className="overflow-x-auto text-[13px]">
										...
										>
											{selectedFile.name === "SKILL.md"
												? skill.content
												: `// Default mock content for ${selectedFile.name}\n\n`}
										</SyntaxHighlighter>
									</div>
								)}
							</div>
		...
		<div className="flex-1 flex h-full bg-theme-base overflow-hidden">
			...
			<div className="flex-1 flex flex-col items-center justify-center bg-theme-base p-6 text-center">
				<div className="w-16 h-16 bg-theme-surface rounded-2xl flex items-center justify-center mb-6 shadow-sm border border-theme-border">
					<div className="w-8 h-8 text-theme-text">{selectedConnector.icon}</div>
				</div>
